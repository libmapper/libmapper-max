...
#ifdef MAXMSP
    mapper_signal msig = mdev_get_input_by_name(x->device, atom_getsym(argv)->s_name, 0);
    if (!msig) {
        post("Error setting value: signal named \"%s\" does not exist!", atom_getsym(argv)->s_name);
        return;
    }
#else
    mapper_signal msig = mdev_get_input_by_name(x->device, (argv)->a_w.w_symbol->s_name, 0);
    if (!msig) {
        post("Error setting value: signal named \"%s\" does not exist!", (argv)->a_w.w_symbol->s_name);
        return;
    }
#endif
